 ...
 * track of the next byte to be supplied by the <code>read</code> method.
 * <p>
 * Closing a <tt>LockFreeMemoryInputStream</tt> has no effect. The methods in
 ...
 * @since JDK1.0
 */
public class LockFreeMemoryInputStream extends InputStream {
	...
	/**
	 * The currently marked position in the stream. LockFreeMemoryInputStream
	...
	/**
	 * Creates a <code>LockFreeMemoryInputStream</code> so that it uses
	 ...
	 *            the input buffer.
	 */
	public LockFreeMemoryInputStream(byte buf[]) {
		this.buf = buf;
		this.pos = 0;
	...
	/**
	 * Creates <code>LockFreeMemoryInputStream</code> that uses <code>buf</code>
	 ...
	 */
	public LockFreeMemoryInputStream(byte buf[], int offset, int length) {
		this.buf = buf;
		this.pos = offset;
	 ...
	 * <code>LockFreeMemoryInputStream</code> always returns <code>true</code>.
	 * 
	 * @since JDK1.1
	 */
	@Override
	...
	/**
	 * Set the current marked position in the stream. LockFreeMemoryInputStream
	 * objects are marked at position zero by default when constructed. They may
	...
	/**
	 * Closing a <tt>LockFreeMemoryInputStream</tt> has no effect. The methods
